package pl.grm.lobby.server.database;
...
public class MySQLUserManager {
	...
	public MySQLUserManager(MySQLConnector connector) {
		if (connector == null) { throw new InvalidParameterException("Connector is null"); }
	...
	public boolean addUser(User user, String passwd) {
		...
		String insertQuery = "INSERT INTO `javaserver`.`users` "
				+ "(`username`, `password`, `active`, `activated`, `registered`, `access_level`, `mail`) " + "VALUES ('"
				+ user.getName() + "', '" + passwd + "', '0', '0', '" + date + "', '0', '" + user.getMail() + "')";
	...
	public boolean deleteUser(User user) {
		String deleteQuery = "DELETE FROM `javaserver`.`users` WHERE username = '" + user.getName() + "'";
		Statement statement = null;
		ResultSet rS = null;
		try {
			if (userExists(user)) {
	...
	public boolean userExists(String username) throws SQLException {
		...
		ResultSet rS = null;
		try {
			String selectQuery = "SELECT username FROM `javaserver`.`users` WHERE username = '" + username + "'";
			statement = connection.createStatement();
	...
	public boolean isPasswordCorrect(String name, String psswd) {
		Statement statement = null;
		ResultSet rS = null;
		String SQuery = "SELECT username, password FROM `javaserver`.`users` WHERE username = '" + name
				+ "' AND password = '" + psswd + "'";
		try {
			if (userExists(name)) {
				statement = connection.createStatement();
