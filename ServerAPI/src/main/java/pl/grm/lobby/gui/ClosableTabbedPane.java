...
public class ClosableTabbedPane extends JTabbedPane {
	...
	private class TabCloseUI implements MouseListener, MouseMotionListener {
		...
		private Rectangle rectangle = new Rectangle(0, 0, this.width,
				this.height);
		...
		public void mouseReleased(MouseEvent me) {
			if (this.closeUnderMouse(me.getX(), me.getY())) {
				boolean isToCloseTab = ClosableTabbedPane.this
						.tabAboutToClose(this.selectedTab);
		...
		public void paint(Graphics g) {
			...
			for (int j = 0; j < tabCount; j++) {
				if (this.tabbedPane.getComponent(j).isShowing()) {
					int x = (this.tabbedPane.getBoundsAt(j).x + this.tabbedPane
		...
		private void drawClose(Graphics g, int x, int y) {
			if ((this.tabbedPane != null)
					&& (this.tabbedPane.getTabCount() > 0)) {
				Graphics2D g2 = (Graphics2D) g;
				this.drawColored(g2, this.isUnderMouse(x, y)
						? Color.RED
						: Color.WHITE, x, y);
			}
		}
		...
		private boolean isUnderMouse(int x, int y) {
			if ((Math.abs(x - this.meX) < this.width)
					&& (Math.abs(y - this.meY) < this.height)) { return true; }
			return false;
		}
	...
	public boolean tabAboutToClose(int tabIndex) {
		if ((this.getComponentCount() != 0)
				&& (this.getComponentAt(tabIndex) != null)) { return true; }
		return false;
	}
